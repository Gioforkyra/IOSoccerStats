...
function serverFlag(name: string | null): string {
  if (!name) return "";
  // Parse [REGION/CC] pattern e.g. "[EU/FR]", "[EU/DE]", "[NA/US]"
  const bracketMatch = name.match(/\[([^\]]+)\]/);
  if (bracketMatch) {
    const parts = bracketMatch[1].split("/");
    const code = parts[parts.length - 1].trim().toUpperCase();
    // Special city/datacenter codes → country
    if (code === "AMS") return "🇳🇱";
    if (code === "UK") return "🇬🇧";
    if (code === "EU") return "🇪🇺";
    if (code === "NA") return "🇺🇸";
    // Standard 2-letter ISO country code → flag emoji
    if (/^[A-Z]{2}$/.test(code)) {
...
export default async function TournamentDetailPage({
  ...
  const standings = standingsRaw as Awaited<ReturnType<typeof getTournamentStandings>>;
  ...
  function pageUrl(p: number) {
    return `/tournaments/${tournamentId}?page=${p}`;
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className={`gap-6 ${standings.length > 0 ? "grid lg:grid-cols-[1fr_480px]" : ""}`}>
        {/* Matches */}
        <div>
          <h2 className="font-display font-700 text-lg tracking-wider text-chalk-100 uppercase mb-3">
            Matches <span className="text-chalk-400 text-sm font-body normal-case">({totalMatches})</span>
          </h2>
          {matches.length === 0 ? (
                ...
                <div className="flex items-center justify-end gap-1.5 mt-4">
                  {currentPage > 1 && (
                    <Link href={pageUrl(currentPage - 1)}
                      className="px-3 py-1.5 rounded border border-chalk-100/10 text-chalk-400 font-mono text-xs hover:border-[#F4119E]/40 hover:text-[#F4119E] transition-colors">
                      &lt;
                    </Link>
                  )}
                  {pageNumbers.map((p, idx) =>
                    p === "..." ? (
                      <span key={`ellipsis-${idx}`} className="px-2 text-chalk-500 font-mono text-xs">&hellip;</span>
                    ) : (
                      <Link
                        key={p}
                        href={pageUrl(p)}
                        className={`px-3 py-1.5 rounded border font-mono text-xs transition-colors ${
                          p === currentPage
                            ? "border-[#F4119E] bg-[#F4119E] text-white"
          ...
          <div>
            ...
            <div className="rounded-lg border border-chalk-100/8 bg-pitch-900/40 overflow-hidden">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {standings.map((s, i) => {
                    const logoUrl = s.badgeImageUrl
                      ? `/api/img?url=${encodeURIComponent(s.badgeImageUrl)}`
                      : null;
                    const isWinner = tournament.winningTeamId === s.teamId;
                    const form = s.form ?? [];
                    // form values: 0=W, 1=D, 2=L, other=?
                    ...
                    return (
                      <tr key={s.teamId} className={`border-t border-chalk-100/5 hover:bg-[#F4119E]/10 transition-colors ${i % 2 === 0 ? "bg-black/[0.03]" : ""}`}>
                        ...
                        <td className="px-2 py-2">
                          <div className="flex gap-0.5">
                            {Array.from({ length: 5 }).map((_, fi) => {
                              const val = form[fi];
                              const fb = val !== undefined && val in FORM_LABELS
                                ? FORM_LABELS[val]
                                : { label: "?", cls: "bg-chalk-100/10 text-chalk-500" };
                              return (
                                <span key={fi} className={`inline-flex items-center justify-center w-5 h-5 rounded text-[9px] font-mono font-700 ${fb.cls}`}>
                                  {fb.label}
                                </span>
                              );
                            })}
                        ...
                        <td className="px-3 py-2 text-right">
                          <Link
                            href={`/teams/${s.teamId}`}
                            className="inline-block text-[10px] font-mono text-chalk-400 border border-chalk-100/10 px-2 py-1 rounded hover:border-[#F4119E]/40 hover:text-[#F4119E] transition-colors whitespace-nowrap"
                          >
                            VIEW PROFILE
                          </Link>
                        </td>
