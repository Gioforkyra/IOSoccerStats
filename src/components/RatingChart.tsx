...
export default function RatingChart({ points }: { points: Point[] }) {
  ...
  const lastSegColor = segColor(points[points.length - 2]?.rating ?? current, current);
      ...
      <div className="rounded-lg border border-chalk-100/8 bg-pitch-900/40 p-4">
        {points.length === 1 ? (
          <div className="text-xs font-mono text-chalk-400 py-2">
            First snapshot recorded — chart will populate over time.
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={120}>
            <LineChart data={points} margin={{ top: 14, right: 14, left: 0, bottom: 0 }}>
              <XAxis
                dataKey="date"
                tickFormatter={(v) => {
                  const d = new Date(v);
              ...
              {/* Invisible line so recharts sets up the coordinate system */}
              <Line type="linear" dataKey="rating" stroke="transparent" dot={false} activeDot={false} isAnimationActive={false} />
              <Customized component={(props: any) => <SegmentLines {...props} points={points} />} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
