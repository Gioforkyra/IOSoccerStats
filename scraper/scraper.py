...
def parse_match_info(raw: dict) -> Optional[dict]:
    ...
    return {
        "id":           raw["id"],
        "kick_off":     kick_off,
        "home_score":   ms.get("homeGoals", 0) or 0,
        "away_score":   ms.get("awayGoals", 0) or 0,
...
def parse_player_stats(items: list, match_id: int, potm_steam_id: Optional[str], name_side_lookup: dict) -> tuple[list, list]:
    ...
    for item in items:
        ...
        stats.append({
            ...
            "team_side":                 side,
            "position":                  (item.get("position") or {}).get("name") if isinstance(item.get("position"), dict) else None,
